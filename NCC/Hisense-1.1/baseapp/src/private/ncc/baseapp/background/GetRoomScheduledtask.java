...
public class GetRoomScheduledtask implements IBackgroundWorkPlugin {
	...
	public PreAlertObject executeTask(BgWorkingContext arg0) throws BusinessException {
		...
		try {
			JSONObject jsons = service.httpsSendGet(roomurl+"?"+"pageIndex=1&pageSize=1000&beginTime="+beginTime+"&endTime="+endTime);
			...
			for (int i = 0; i < pageData.size(); i++) {
	            ...
	            List<DefdocVO> list1 = (List<DefdocVO>) query.retrieveByClause(DefdocVO.class, "nvl(dr,0)=0 and (def1='"+map.get("roomId")+"' or code = '"+map.get("roomNumber")+"')");
	            ...
	            if (defdocVO == null) {
					...
					defdocVO.setName((String) map.get("roomNumber"));
					defdocVO.setCode((String) map.get("roomNumber"));//房号
					defdocVO.setShortname((String) map.get("roomShortName"));//房间简称
