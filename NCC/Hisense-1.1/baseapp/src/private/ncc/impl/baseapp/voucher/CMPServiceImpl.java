...
import ncc.baseapp.utils.IHttpUtils;
...
public class CMPServiceImpl implements ICMPService{
	...
	@Override
	public String saveCMP(String param) {//api新增数据方法
	...
	@Override
	public String delCMP(String param) {//api删除方法
		OpenAPIUtis utils =  new OpenAPIUtis();
	...
	@Override
	public String writeBack2NC65(JSONObject json) {//结算单签字回写nc65
		IHttpUtils https = NCLocator.getInstance().lookup(IHttpUtils.class);
...
//		JSONObject param1 = JSONObject.parseObject(param);
		String result = "";
		try {
			result = https.doPost_RequiresNew(nc65ip,null, JSONObject.toJSONString(json));
			//result = uaphttp(nc65ip, json);
		} catch (Exception e) {
	...
	public String uaphttp(String url, JSONObject obj) {//post请求 [只适用ncc调nc65]
	...
	@Override
	public String settlementBack2NC65(JSONObject json) {//结算单 结算回写nc65
    ...
    public JSONObject httpsSendGet(String hsurl) throws Exception {//HTTPS GET请求
    	 URL url = null;
